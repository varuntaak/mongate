...
var mongate = (function() {
	...
	var doTransaction = function (txn, callback) {
		if (!txn.name ||
			...
			try {
				...
				createTransaction.apply(this, [transaction, function (err, txn) {
					var interval = setTimeInterval (coordinateTransaction, '10');
					processJobs.apply(that, [transaction.jobs, function (err, status){
						console.log(status);
					}]);
					console.log(txn);
					callback(err, txn);
				}]);
				// createTransaction(transaction, function (err, txn) {
				// 	// var interval = setTimeInterval (coor )
				// 	console.log(txn);
				// 	callback(err, txn);
				// });			
			} catch (e) {
	...
	var coordinateTransaction = function () {
	...
	var createTransaction = function () {
		...
		var dbRequestToCreateTxn = new DBRequest({
          'collectionName' : Collections.txns,
          'document'  : txn
        });

...
var mongate = (function() {
	...
	var getDistinctValues = function (dbRequest, callback) {
		if (dbRequest.collectionName === undefined ||
			...
			this.getDbClient(dbName, function (err, dbClient) {
				var collection = that.getDbCollection(dbClient, dbRequest.collectionName);
				collection.distinct(dbRequest.field, dbRequest.query, function(err, values) {
		       		if (err) {
		       			callback(err);
		       			return;
		       		} else {
		       			callback(undefined, values.sort({$natural: -1}));
		       		}
				});
			})
		}
